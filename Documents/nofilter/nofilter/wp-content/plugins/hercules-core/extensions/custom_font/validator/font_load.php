...
if (!class_exists('Redux_Validation_font_load')) {
	...
	class Redux_Validation_font_load {
		...
		function __construct($parent, $field, &$value, $current) {
			...
			$this->field['msg'] = ( isset($this->field['msg']) ) ? $this->field['msg'] : esc_html__('You must provide a numerical value for this option.', 'buzzblogpro');
			...
			if (!empty($options) && isset($options[0]["attachment_id"]) && !empty($options[0]["attachment_id"])) {
				if ($value["attachment_id"] == "") {
					$this->field["msg"] = esc_html__("No attached file", 'buzzblogpro');
...
//				die();
			...
			if (isset($this->value["attachment_id"]) && !empty($this->value["attachment_id"])) {
				...
				if (Custom_Font_Validator::checkIfFontExist($this->value["attachment_id"])) {
					...
					if ($this->count > 0) {
						$this->field["msg"] = esc_html__("Font already exists", 'buzzblogpro');
						$this->error = $this->field;
						unset($this->value);
					}
				...
				if (!$format) {
					$this->field["msg"] = esc_html__("Your must download .zip format file", 'buzzblogpro');
					$this->error = $this->field;
					unset($this->value);
					return false;
				}
...
//				die();
//				if (count($options) >= 1) {
//					$this->field["msg"] = esc_html__("No attached file2", 'buzzblogpro');
//					$this->error = $this->field;
//				}
			}
		}
		...
		private function upload() {
			...
			if (!file_exists($temp_folder)) {
				...
				if ($has_font) {
					$this->field["msg"] = esc_html__("Font exist", 'buzzblogpro');
					$this->error = $this->field;
					unset($this->value);
					Custom_Font_Validator::remove_folder($temp_folder);
					return false;
				...
				if (trim($font_file_name) == '') {
					$this->field["msg"] = esc_html__("No found font file", 'buzzblogpro');
					$this->error = $this->field;
					unset($this->value);
					Custom_Font_Validator::remove_folder($temp_folder);
				} else {
					if (file_exists($this->inst->upload_dir . $this->value["attachment_id"] . '/' . $font_file_name)) {
						if ($this->count > 0) {
							$this->field["msg"] = esc_html__('There is already a font with the same name', 'buzzblogpro');
							$this->error = $this->field;
							unset($this->value);
							Custom_Font_Validator::remove_folder($temp_folder);
						}
						...
						if (!Custom_Font_Validator::copy_zip_fontfiles($temp_folder, $this->inst->upload_dir . $this->value["attachment_id"] . '/' . $font_file_name)) {
							$this->field["msg"] = esc_html__('Error during file upload', 'buzzblogpro');
							$this->error = $this->field;
							unset($this->value);
						}
					}
