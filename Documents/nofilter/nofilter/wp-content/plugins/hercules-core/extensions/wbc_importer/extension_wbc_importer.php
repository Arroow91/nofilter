...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
        ...
        public function ajax_importer() {
			...
			$uninstalling = false;
			    if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-uninstall" ) {
                $uninstalling = true;
				
				
				if ( true === $uninstalling ) {
				
				$this->buzzblogpro_uninstall_demo_attachments();
		...
		public function buzzblogpro_uninstall_demo_attachments(){
	...
	$args2 = array(
		'post_type' => array('page', 'post', 'gallery', 'faq'),
		'posts_per_page' => '-1',
		'meta_query' => array(
			array(
				'key'     => 'buzzblogpro_demo_content_key',
				'value'   => 1,
			),
		)
	...
	$args3 = array(
		...
		'meta_query'	=> array(
			array(
				'key'     => 'buzzblogpro_demo_term_key',
				'value'   => 1,
			),
		)
	);
	$terms1 = get_terms( $args3 );
	...
	$menu_names = array('Main menu', 'primary-menu', 'footer_menu', 'top-menu', 'mobile_menu', 'split-right-menu', 'Split right menu');
	foreach( $menu_names as $menu ) {
		wp_delete_nav_menu($menu);
		}  
	//delete_option( 'buzzblogpro_options' );
