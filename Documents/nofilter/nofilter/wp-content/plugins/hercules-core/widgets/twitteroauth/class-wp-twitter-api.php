...
class Wp_Twitter_Api {
	...
	$bearer_token_option = 'buzzblogpro_bearer_token';
	...
	private function get_bearer_token() {
		...
		if ( is_wp_error( $response ) || 200 != $response['response']['code'] )
			return $this->bail( apply_filters( 'buzzblogpro_twitter_missing_key_message', esc_html__( 'Error: access keys missing in Appearance -> Theme Options -> Twitter Settings', 'buzzblogpro' ) ), $response );
	...
	public function query( $query, $query_args = array(), $stop = false ) {
		...
		if ( is_wp_error( $response ) || 200 != $response['response']['code'] ){
			if ( !$stop ){
				...
				return $this->bail( esc_html__( 'Bearer Token is good, check your query', 'buzzblogpro' ), $response );
			}
		}
