<?php
class buzzblogpro_PostsBlock2 extends WP_Widget {

	public function __construct() {

		$widget_ops = array(
			'classname' => 'buzzblogpro_PostsBlock2', 
			'description' => esc_html__('A widget that display a posts block', 'buzzblogpro') 
		);
        
		parent::__construct(
			'buzzblogpro_PostsBlock2',
			esc_html__('Hercules - Posts Block 2', 'buzzblogpro'),
			$widget_ops
		);
...
$excerpt_count = isset($instance['excerpt_length']) ? $instance['excerpt_length'] : '12';
    ...
    case "Image":
	 ...
	 case "Video":
		...
		$query_block2 = new WP_Query( $arguments );
		if ( $query_block2->have_posts() ) :
		...
		if (isset($instance['show_category_link']) && $instance['show_category_link'] != 'no' ) {
			...
			while ( $query_block2->have_posts() ) : $query_block2->the_post();
...
if (isset($instance['first_big_image']) && $instance['first_big_image'] == 'yes' ) {
$small = 'small-post';
}else{
...
if ($columns == 4 && $i > 3) {$small = 'small-post';}
    ...
    echo '<div class="cat_post_item-'.esc_attr($i).' '.esc_attr($small).' col-xs-12 col-sm-6 col-md-'.$column.'">';
    }
	
	echo '<div class="post-grid-block post">';
	
		$argsy = array(
		'width'          => isset($instance['thumb_width']) ? $instance['thumb_width'] : 359,
...
echo '</div>';							
	...
	<?php 
	$whattoshow = array();
	if (isset($instance['show_author']) && $instance['show_author'] != 'no' && $small != 'small-post' ) {
	$whattoshow[] = 'author';
	}
	if (isset($instance['show_date']) && $instance['show_date'] != 'no' ) {
	$whattoshow[] = 'date';
	}
	if (isset($instance['comment_num']) && $instance['comment_num'] != 'no' && $small != 'small-post' ) {
	$whattoshow[] = 'comments';
	}
    buzzblogpro_post_meta($whattoshow, true, 'meta-space-top'); 
	echo '<div class="clear"></div></header>';
	?>
	<?php if($small != 'small-post'){ ?>
...
public function form($instance) {
  ...
  $defaults = array(
			'title' => '', 'columns' => '3','rows' => '1','mostpopular' => 'None','postformat' => 'All','widget_categories' => array(), 'exclude' => 'exclude', 'thumb_width' => '359', 'thumb_height'=> '348','comment_num' => 'yes','show_title' => 'yes', 'first_big_image' => 'no','show_date' => 'yes','show_category' => 'yes', 'show_category_link' => 'yes', 'show_author' => 'yes', 'excerpt_length' => '12', 'more_link' => 'yes','custom_posts' => '' 
			);
...
<fieldset style="border:1px solid #F1F1F1; padding:10px 10px 0; margin-bottom:1em;">
  ...
  <p>
        <label for="<?php echo esc_attr($this->get_field_id("first_big_image")); ?>">
    <?php esc_html_e( 'Do you want to turn off a large photo for the first row?', 'buzzblogpro' ); ?>
	<select id="<?php echo esc_attr($this->get_field_id("first_big_image")); ?>" name="<?php echo esc_attr($this->get_field_name("first_big_image")); ?>">
      <?php
        $first_big_image_options = array('yes', 'no');
            foreach ($first_big_image_options as $option) {
              echo '<option value="' . $option . '" id="' . $option . '"', $instance["first_big_image"] == $option ? ' selected="selected"' : '', '>', $option, '</option>';
            } ?>
    </select>
  </label>
  </p>
