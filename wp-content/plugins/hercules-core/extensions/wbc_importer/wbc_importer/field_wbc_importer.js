...
(function($) {
    ...
    $.redux.wbc_importer = function() {
		...
		$('.wrap-importer.theme.imported, #wbc-importer-uninstall').on( 'click', function(e) {
		e.preventDefault();
var message  = 'Uninstall demo contents?';
            var parent = jQuery(this);
			var uninstalling = false;
if (e.target.id == 'wbc-importer-uninstall') {
                uninstalling = true;
                message  = 'Uninstall demo contents?';
            ...
            if (parent.hasClass('imported') && uninstalling == false) return;
...
data.action = "redux_wbc_importer";
           data.nonce = parent.attr("data-nonce");
            data.type = 'import-demo-uninstall';
           data.wbc_import = (uninstalling == true) ? 'uninstalling' : ' ';
            parent.find('.wbc_image').css('opacity', '0.5');
            parent.find('.spinner').css('display', 'inline-block');
			 parent.find('.importer-button-uninstall').hide();
			 parent.find('#wbc-importer-reimport').hide();
...
parent.find('.spinner').css('display', 'none');
                    if (uninstalling == false) {
                        alert('uninstalling false');
                    }
					parent.find('.importer-button-uninstall').removeClass('button-primary').addClass('button').text('Success').show();
                   location.reload(true);
                } else {
				 parent.find('.importer-button-uninstall').show();
                    alert('There was an error uninstalling demo content: \n\n' + response.replace(/(<([^>]+)>)/gi, ""));
                }
            });
