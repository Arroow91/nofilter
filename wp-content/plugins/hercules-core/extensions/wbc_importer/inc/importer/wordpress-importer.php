...
<?php
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($cat['term_id']) ) {
					...
					//Theme Change
					add_term_meta( (int) $id, 'buzzblogpro_demo_term_key', 1);
					}
			} else {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				if ( isset($tag['term_id']) ){
					...
					//Theme Change
					add_term_meta( (int) $id['term_id'], 'buzzblogpro_demo_term_key', 1);
				}
			} else {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( 'attachment' == $postdata['post_type'] ) {
					...
					update_post_meta( $post_id, '_wp_attachment_image_alt', 'demo-images' );
				} else {
			...
			if ( ! empty( $post['postmeta'] ) ) {
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						...
						add_post_meta( $post_id, 'buzzblogpro_demo_content_key', 1 );
	 ...
	 */
		function process_menu_item( $item ) {
			...
			if( isset( $_buzzblogpro_mega_menu_item ) ) {
				update_post_meta( $id, '_buzzblogpro_mega_menu_item', $_buzzblogpro_mega_menu_item );
			}
			if( isset( $_buzzblogpro_mega_menu_column ) ) {
				update_post_meta( $id, '_buzzblogpro_mega_menu_column', $_buzzblogpro_mega_menu_column );
			}
			if( isset( $_buzzblogpro_mega_menu_column_sub_item ) ) {
				update_post_meta( $id, '_buzzblogpro_mega_menu_column_sub_item', $_buzzblogpro_mega_menu_column_sub_item );
			}
			if( isset( $_buzzblogpro_mega_menu_dual ) ) {
				update_post_meta( $id, '_buzzblogpro_mega_menu_dual', $_buzzblogpro_mega_menu_dual );
			}
			if( isset( $_buzzblogpro_menu_widget ) ) {
				update_post_meta( $id, '_buzzblogpro_menu_widget', maybe_unserialize( $_buzzblogpro_menu_widget ) );
			}
			if( isset( $_buzzblogpro_dropdown_columns ) ) {
				update_post_meta( $id, '_buzzblogpro_dropdown_columns', $_buzzblogpro_dropdown_columns );
			}
		}
	...
	function remap_featured_images() {
		...
		foreach ( $this->featured_images as $post_id => $value ) {
			if ( isset( $this->processed_posts[$value] ) ) {
				...
				update_post_meta( $new_id, '_wp_attachment_image_alt', 'demo-images' );	
	 ...
	 * @return int 60
	 */
	function bump_request_timeout( $val = 300 ) {
		return 300;
	}
