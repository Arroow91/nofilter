...
if (is_tag()) {
$blog_cat_sidebar_pos = buzzblogpro_getVariable('blog_cat_sidebar_pos') ? buzzblogpro_getVariable('blog_cat_sidebar_pos') : 'masonry2' ;
}else{
...
$blog_cat_sidebar_pos = $individual_layout =='inherit' ? buzzblogpro_getVariable('blog_cat_sidebar_pos') : $individual_layout;
}
$blog_grid_special_post = buzzblogpro_getVariable('blog_grid_special_post');

switch ($blog_cat_sidebar_pos) {
		case 'masonry2':
			$blog_class = '6';
			$counter_set = '3';
		break;
...
if ($counter % $counter_set == 0 && $blog_grid_special_post == 'yes') { ?>
...
<?php }else{ ?>
<div id="post-<?php the_ID(); ?>" class="grid-item ajax-post-wrapper block col-xs-12 col-sm-<?php echo esc_attr( $blog_class); ?> col-md-<?php echo esc_attr( $blog_class); ?>" > 
